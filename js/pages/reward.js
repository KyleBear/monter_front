...
export const initRewardPage = (container) => {
    ...
    container.innerHTML = `
        <div class="reward-info">
            ...
            리워드 상품을 확인하고 이미지 태그를 입력할 수 있습니다.
        </div>
...
const loadRewardList = async () => {
    try {
        ...
        if (response.ok) {
            ...
            console.log('리워드 개수:', rewards.length);
...
const renderRewardList = (rewards) => {
    ...
    container.innerHTML = rewards.map((reward, index) => {
        ...
        const productUrl = reward.product_url || reward.search_url || '#';
            ...
            <div class="reward-card" data-reward-id="${rewardId}">
                <div class="reward-card-content">
                    <div class="reward-image-section">
                        ...
                        <a href="${productUrl}" target="_blank" class="reward-link-btn">상품 누르러 가기</a>
                    </div>
...
const submitImageTag = async (rewardId, imageTag) => {
    try {
        ...
        if (response.ok) {
            ...
            alert(`이미지 태그 저장 실패: ${errorData.message || errorData.detail || '서버 오류가 발생했습니다.'}`);
        }
    } catch (error) {
